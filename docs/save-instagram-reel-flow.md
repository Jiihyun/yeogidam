...
캡션을 얻으면 비용이 낮고 결정적인 규칙부터 실행합니다.

1. 한국 도로명 주소 정규식
   - 예: `서울 서대문구 연희맛로 17-63`
2. 핀 라인 장소명
   - 예: `📍보연희` → `보연희`
3. Gemini 구조화 추출
   - 앞 단계에서 장소명을 얻지 못했을 때만 호출
   - 결과: `{ placeName, region }`

Naver 검색 쿼리는 중복을 제거한 뒤 아래 순서로 생성합니다.
...
이 사례는 `name="description"` 메타 지원과 oEmbed JSON 추출, `📍` 장소명 추출이 추가된 계기이자 회귀 테스트 기준입니다.
